...
public class App {
	...
	public static void main(String[] args) {
		try {
			...
			ProgressConsole progress = null;
			if (!cliHelper.isQuiet()) {
			...
			if (cliHelper.hasImportOption()) {
				...
				Collection<XcpParameter> updatedPrmList = configTool.importConfig(repoName, username, password,
						inputStream, false, excluder, progress);
				int count = updatedPrmList.size();
				progress.progress("Parameter(s) updated: " + Integer.toString(count) , "", null);
			} else {
